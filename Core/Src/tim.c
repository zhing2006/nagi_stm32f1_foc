...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  ...
  // If the motor is not calibrated, skip the speed calculation.
  ...
  if (first_run) {
    last_encoder_angle = g_motor.encoder_angle;
    first_run = false;
  } else {
    float encoder_angle = g_motor.encoder_angle;
    float diff_angle = nagi_foc_motor_normalize_angle_2pi(encoder_angle - last_encoder_angle);
    last_encoder_angle = encoder_angle;
    float speed = diff_angle * SPEED_CALC_FREQ;
    g_motor.speed = kalman_filter_1d_update(&g_speed_filter, speed);
    g_motor.is_speed_ready = true;
  }
}
/* USER CODE END 0 */
